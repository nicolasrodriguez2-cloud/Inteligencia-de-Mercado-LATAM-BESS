...
- Actualizar mapa_gaps_mercado2026.md dentro de la misma rama, reflejando
  lo que esta corrida encontró/cerró. Solo puede documentar estado (qué
  se investigó, qué se encontró, qué falta) — NUNCA una decisión
  estructural (reordenar categorías, renombrar/eliminar/fusionar filas).
  Esas quedan señalizadas como "pendiente decisión de Nicolás" dentro
  del propio texto del gap map.
- Generar log_[país]_[fecha].md (fila | valor anterior | valor nuevo |
...
- Al guardar el xlsx: cargar con data_only=False, usar fórmulas (no
  hardcodear totales), correr recálculo y verificar que las fórmulas
  preexistentes de otras hojas sigan intactas.
- Abrir un Pull Request contra main con: el oficial editado + el gap map
  editado + el log + las tres copias fechadas en /historial/ — todo en
  el mismo PR, para que un solo merge deje todo sincronizado.
- NUNCA mergear solo — Nicolás revisa y mergea directamente en GitHub
  cuando el PR le alcance para confiar, sin necesidad de traerlo a una
  sesión de Chat.
