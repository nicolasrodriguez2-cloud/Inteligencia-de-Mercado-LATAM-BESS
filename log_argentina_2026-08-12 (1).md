...
**Candidate file:** `candidato_argentina_2026-08-12.xlsx`
**Base file:** `BESS_POTENTIAL_CLIENTS_v2 1.xlsx` (unmodified in this branch)
...
on what date it was searched. **Decision for Nicolás:** if you would rather see the gap note
inside the numeric cell itself, that is a one-line change, but it makes the column mixed-type.
...
| 18 | FALABELLA | **DATA-QUALITY FLAG.** Falabella exited Argentina in 2021 — last stores closed Apr-2021, e-commerce closed 31-May-2021, definitive exit 01-Jun-2021. No operating load in the country, therefore no BESS potential as listed. | Infobae 01-Jun-2021; La Nación 06-Apr-2021; Cooperativa.cl 01-Jun-2021 — accessed 2026-08-12 | Confirmed (b) |
...
- **File handling:** the candidate was produced by editing the Argentina worksheet XML inside
  the workbook package, rather than through a full openpyxl re-save. A round-trip through
  openpyxl was tested first and **silently dropped all five embedded images** (the logos in
  `xl/media/`) along with the shared-string table. The method used changes only
  `xl/worksheets/sheet2.xml` and `xl/styles.xml`; the other 30 parts of the package are
  byte-identical to the official file. Verified after the build: identical part list, all XML
  well-formed, every pre-existing formula intact (`Argentina!E7`, `E9`, `E97=SUM(E87:E96)`;
  `Chile!E8`, `E9`) together with its cached value, and **zero new formulas introduced**.
- **Recalculation:** not applicable and not run — this change adds no formulas and touches no
  existing one, so all cached values remain those of the official file. For the record,
  LibreOffice could not be used to verify anyway: `soffice` in this Routine environment times
  out (>8 min) even on a two-cell test workbook, so no recalculation step is available here.
  Worth knowing before a future run tries to write formulas from a Routine.
- **Branch:** this run was pushed to `claude/modest-dijkstra-rpxwbq`, the branch assigned by
  the scheduler, instead of the `claude/[país]-[fecha]` convention in CLAUDE.md
  (`claude/argentina-2026-08-12`). The branch name is imposed by the Routine harness and could
  not be chosen. Flagging it so the naming convention is not assumed broken.

---

## 5. Suggested update to `mapa_gaps_mercado2026.md`

Not applied here — the gap map is edited in Chat sessions only. Proposed wording once this PR
is reviewed:

- **Argentina → Commercial Clients:** 🟡 → ✅ *(as researched)* — "16/16 clients specifically
  searched 2026-08-12; 0/16 with a confirmed BESS. Documented confirmed gap, with source class,
  source and access date per row. This is the real public-data ceiling, not pending work.
  3 rows flagged for identity review (Yamana Gold, Falabella, Movistar)."
- **Completeness-per-client table, Argentina → Commercial Clients:** "0/16 with a numeric
  figure — but now 16/16 with a documented, sourced status" (replacing "only name and category").
- **Argentina → Competitors:** add "manufacturer–project pair found 2026-08-12 (Trina Storage →
  Central Dock Sud / YPF Luz, Alma Sur 90 MW / 481 MWh) — pending its own run, see
  `log_argentina_2026-08-12.md` §3.1."
